...
public:
  Hex();
  Hex(HexCoords coords, float size, sf::Color fillColor, sf::Color outlineColor,
      int outlineThickness);
  ~Hex() {}
  sf::Color getFillColor() const { return _fillColor; }

  sf::Color getOutlineColor() const { return _outlineColor; }

  int getOutlineThickness() const { return _outlineThickness; }
  ...
  bool operator==(const Hex &hex) const {
    return (this->_positionHex.q == hex._positionHex.q &&
            this->_positionHex.r == hex._positionHex.r);
  }
  HexCoords getCoords() const { return _positionHex; }

  virtual void updatePosition() {
    this->setPosition(axialToScreen(_positionHex, _size));
  }

  void addEntity(Interactable *entity) { _interactables.push_back(entity); }

  void removeEntity(Interactable *entity) {
    _interactables.erase(
        std::remove(_interactables.begin(), _interactables.end(), entity),
        _interactables.end());
    std::cout << "Removed entity from hex" << std::endl;
  }

  std::vector<Interactable *> getEntities() const { return _interactables; }

  void toggleSelectedEntity() {
    std::cout << "switching between " << _interactables.size() << " entities"
              << std::endl;
