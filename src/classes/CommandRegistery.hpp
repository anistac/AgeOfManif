...
public:
  ...
  static void addCmd(Command *cmd) {
    ...
    commands[std::make_pair(t1, t2)] = cmd;
  }
  template <typename I1, typename I2>
  static Command* findAvailableCmds(const I1 init1, const I2 init2) {
    auto it = commands.find(std::make_pair(std::type_index(init1), std::type_index(init2)));
    if (it != commands.end()) {
      return it->second;
    } else {
...
private:
  static std::unordered_map<std::pair<std::type_index, std::type_index>, Command*, CmdTypeHash, CmdTypeEq> commands;
};
