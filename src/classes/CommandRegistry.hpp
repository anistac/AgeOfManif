...
static void addCmd(std::shared_ptr<Command> cmd) {
  std::type_index t1 = typeid(I1);
  _commands.insert(std::make_pair(std::make_pair(t1, std::type_index(typeid(void))), cmd));
}
...
template <typename I1>
static std::vector<std::shared_ptr<Command>> findAvailableCmds(const I1 init1) {
  std::type_index t1 = typeid(*init1);
  return findCmdsWithInitiators(t1, std::type_index(typeid(void)));
}

// Helper function to find commands with initiators
static std::vector<std::shared_ptr<Command>> findCmdsWithInitiators(std::type_index t1, std::type_index t2) {
