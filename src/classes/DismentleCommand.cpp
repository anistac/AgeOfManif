...
bool DismentleCommand::execute(Interactable *init1, Interactable *init2) {
  RoundAbout *batiment = dynamic_cast<RoundAbout *>(init1) == nullptr
                             ? dynamic_cast<RoundAbout *>(init2)
                             : dynamic_cast<RoundAbout *>(init1);
  Policier *troupe = dynamic_cast<Policier *>(init1) == nullptr
                         ? dynamic_cast<Policier *>(init2)
                         : dynamic_cast<Policier *>(init1);
  ...
  Hex *hex = batiment->getRegion()->getGrid().getHexFromCoords(
      batiment->getHexCoords());
  troupe->move(hex);
